...
async function analyzeAndRouteQuery(
  ...
  const Router = z
    .object({
      logic: z.string().describe("Your reasoning for classifying the query"),
      type: z
        .enum(["more-info", "law", "shareholder-meeting", "general"])
        .describe(
          "The classification type: 'more-info' if you need more information, 'law' if it's about general legal matters, 'shareholder-meeting' if it's specifically about shareholder meetings, 'general' if it's unrelated to law"
        ),
    })
    .describe("Classify user query into one of the four types.");
  const response = (await model
    .withStructuredOutput(Router)
    .invoke(messages)) as {
    logic: string;
...
function routeQuery(
  ...
  if (type === "law" || type === "shareholder-meeting") {
